...
function PricingCardV2({ plan }: { plan: PricingPlan }) {
  return (
    <div className={`relative flex flex-col h-full bg-white rounded-2xl v2-comic-border p-6 md:p-7 ${plan.isPopular ? 'v2-comic-shadow-blue' : 'v2-comic-shadow'}`}>
      {/* Most Popular — starburst badge */}
      {plan.isPopular && (
...
export function PricingSectionV2() {
  ...
  return (
    <section className="bg-[#F8F8FA] py-16 md:py-20 v2-halftone relative">
      <div className="max-w-[1200px] mx-auto px-6 relative z-10">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 lg:gap-8 mb-8">
          {plans.map((plan, index) => (
            <PricingCardV2 key={index} plan={plan} />
          ))}
        </div>
