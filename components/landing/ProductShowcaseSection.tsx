...
function TypewriterSuggestion({ text, source, isActive }: { text: string; source: string; isActive: boolean }) {
  ...
  return (
    <div className={`showcase-glass-strong rounded-2xl border border-white/15 shadow-[0_10px_30px_rgba(0,0,0,0.35),inset_0_1px_0_rgba(114,126,137,0.4)] overflow-hidden transition-opacity duration-500 ${fadingOut ? 'opacity-0' : 'opacity-100'}`}>
      {/* Header */}
      <div className="flex items-center gap-2 px-4 pt-3 pb-1">
        <span className="text-[#2367EE]"><SparkleIcon /></span>
        <span className="text-[11px] font-semibold text-white/60 uppercase tracking-wider">Sayso suggests</span>
      </div>
      {/* Body */}
      <div className="px-4 pb-2">
        <p className="text-[14px] md:text-[15px] leading-relaxed text-white/95 font-medium">
          &quot;{displayedText}&quot;
          {!isDoneTyping && (
      ...
      {/* Source chip */}
      <div className={`px-4 pb-3 transition-opacity duration-300 ${isDoneTyping ? 'opacity-100' : 'opacity-0'}`}>
        <div className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-full bg-white/8 border border-white/10">
          <div className="w-4 h-4 rounded-full bg-[#2367EE]/30 flex items-center justify-center">
            <span className="text-[8px] text-[#2367EE] font-bold">S</span>
          </div>
